# 1 "../gbs8/driver/scr/gbs8_stepper.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../gbs8/driver/scr/gbs8_stepper.c" 2
...
# 5 "../gbs8/bsp/inc\\gbs8_gpio.h" 2
# 74 "../gbs8/bsp/inc\\gbs8_gpio.h"
void GBS_GPIO_Init();
# 5 "../gbs8/driver/inc\\gbs8_stepper.h" 2
...
# 2 "../gbs8/driver/scr/gbs8_stepper.c" 2
...
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.20\\pic\\include\\c90\\stdint.h" 1 3
# 6 "../gbs8/bsp/inc\\gbs8_interrupt.h" 2
# 21 "../gbs8/bsp/inc\\gbs8_interrupt.h"
void EXTI_ISR();
...
# 3 "../gbs8/driver/scr/gbs8_stepper.c" 2

# 1 "../gbs8/bsp/inc\\gbs8_relay.h" 1
# 21 "../gbs8/bsp/inc\\gbs8_relay.h"
void GBS_Relay_Init();
...
void GBS_Stepper_Init()
{
    ...
    GBS_Timer2_Config(1,T2CKPS1,T2OUTPS1,0);
...
uint8_t GBS_Stepper_Planner(sBuffer_t* sBufferX, dir_t dir, rotate_t rotation, speedRm_t v_i, speedRm_t v_m, speedRm_t v_o, accRm_t a_i, accRm_t a_o)
{
    ...
    float t_i = (v_o - v_i) * a_i / (((a_i)*(a_i)) - ((a_o)*(a_o)));

    float v_a = v_i + a_i * t_i;

    steps_t totalSteps = (steps_t)(rotation*(float)200);

    if (v_a<=v_m)
    {
        s_i = totalSteps * s_o * a_i / a_o;
        s_o = totalSteps - s_i;
        v_m_c = 200*10000*(accCnt_t)(v_a / 60.0);
    }
    else
    {
        ...
        v_m_c = 200*10000*(accCnt_t)(v_m / 60.0);
    }


    if (sBufferX->buffer[sBufferX->tail].flag==1)
    {
        sBufferX->buffer[sBufferX->tail].dir = dir;
        sBufferX->buffer[sBufferX->tail].acc = 200*10000*(accCnt_t)(a_i / 60.0);
        sBufferX->buffer[sBufferX->tail].dec = 200*10000*(accCnt_t)(a_o / 60.0);
        ...
        sBufferX->buffer[sBufferX->tail].maxSpeed = 200*10000*(accCnt_t)(v_m / 60.0);
        sBufferX->buffer[sBufferX->tail].flag = 2;
...
void GBS_Stepper_Exe(stepper_t* stepperX, sBuffer_t* sBufferX)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (sBufferX->buffer[sBufferX->head].flag == 3)
            {
                if (sBufferX->buffer[sBufferX->head].acc_until>=0)
                {
                    stepperX->pinState = 1;
                    stepperX->cntsLast = stepperX->cntsLast + 1*((stepperX->cntsLast)*(stepperX->cntsLast)*(stepperX->cntsLast))*sBufferX->buffer[sBufferX->head].acc/((10000)*(10000)) + ((stepperX->cntsLast)*(stepperX->cntsLast)*(stepperX->cntsLast))*((stepperX->cntsLast)*(stepperX->cntsLast))*((sBufferX->buffer[sBufferX->head].acc/((10000)*(10000)))*(sBufferX->buffer[sBufferX->head].acc/((10000)*(10000))));

                    sBufferX->buffer[sBufferX->head].acc_until--;
                    sBufferX->buffer[sBufferX->head].dec_after--;
                }
                else if (sBufferX->buffer[sBufferX->head].dec_after>=0)
                {
                    stepperX->pinState = 1;
                    sBufferX->buffer[sBufferX->head].dec_after--;
                }
...
# 275 "../gbs8/driver/scr/gbs8_stepper.c"
}
...
# 304 "../gbs8/driver/scr/gbs8_stepper.c"
    GBS_Stepper_Update();
}
